...
module.exports = function( config )
{
	...
	{
		return gulp.src( [
				config.gjLibDir + 'components/**/*.svg',
			] )
			.pipe( plugins.newer( config.buildDir + '/' + config.gjLibDir.replace( 'src/', '' ) + 'components' ) )
			.pipe( config.production ? plugins.svgmin() : gutil.noop() )
			.pipe( gulp.dest( config.buildDir + '/' + config.gjLibDir.replace( 'src/', '' ) + 'components' ) )
			.pipe( plugins.connect.reload() )
	} );
	...
	{
		return gulp.src( [
				config.gjLibDir + 'components/**/*.{png,jpg,jpeg,gif}'
			] )
			.pipe( plugins.newer( config.buildDir + '/' + config.gjLibDir.replace( 'src/', '' ) + 'components' ) )
			.pipe( config.production ? plugins.imagemin( { progressive: true, interlaced: true, pngquant: true } ) : gutil.noop() )
			.pipe( gulp.dest( config.buildDir + '/' + config.gjLibDir.replace( 'src/', '' ) + 'components' ) )
