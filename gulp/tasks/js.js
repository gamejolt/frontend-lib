...
module.exports = function(config) {
	...
	function getBowerComponentFiles(component) {
		...
		var componentBower = require(baseDir +
			config.bowerDir +
			component +
			'/.bower.json');
		if (componentBower.main) {
			if (
				_.isString(componentBower.main) &&
				componentBower.main.match(/\.js$/)
			) {
	...
	gulp.task('js:vendor:rollup', function(cb) {
		...
		return gulp
			.src('src/vendor.ts', { read: false, base: 'src' })
	...
	gulp.task(
		'js:vendor',
		gulp.series(gulp.parallel(vendorCommonDepends), function(cb) {
			...
			var files = [config.buildDir + '/tmp/rollup/vendor.js'];
	...
	config.sections.forEach(function(section) {
		...
		gulp.task('ts:' + section, function(cb) {
			...
			return gulp
				.src('src/' + section + '/app.ts', { read: false, base: 'src' })
				.pipe(plugins.rollup(_rollupOptions))
		...
		gulp.task(
			'js:' + section,
			gulp.series(
				gulp.parallel('ts:' + section, 'html2js:' + section + ':partials'),
				function(cb) {
					if (
						config.buildSection &&
						config.buildSection != section &&
	...
	if (config.modules) {
		...
		_.forEach(config.modules, function(moduleDefinition, outputFilename) {
			gulp.task('ts:module:' + outputFilename, function(cb) {
				if (
					config.buildModule &&
					config.buildModule != outputFilename &&
					config.watching == 'watching'
				) {
					cb();
					return;
				...
				var rollupStream = gulp
					.src('src/app' + moduleDefinition.main, { read: false, base: 'src' })
					.pipe(plugins.rollup(_rollupOptions));
			...
			gulp.task(
				'js:module:' + outputFilename,
				gulp.series('ts:module:' + outputFilename, function(cb) {
					if (
						config.buildModule &&
						config.buildModule != outputFilename &&
						config.watching == 'watching'
	...
	gulp.task('js:node:app', function(cb) {
		...
		return gulp.src(src, { base: 'src' }).pipe(gulp.dest(config.buildDir));
	});
