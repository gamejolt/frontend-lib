...
export class AppPopover extends Vue
{
	...
	@Prop( { type: String, default: 'bottom' } ) position: 'top' | 'right' | 'bottom' | 'left';
	...
	show( triggerElement: HTMLElement )
	{
		...
		if ( this.position === 'top' || this.position === 'bottom' ) {
			const extraSpacing = elementStyles.left
				...
				: 0;
			this.arrowLeft = triggerLeft + Math.min( triggerWidth / 2, popoverWidth / 2 ) - extraSpacing + 'px';
		}
		else if ( this.position === 'left' || this.position === 'right' ) {
			const extraSpacing = elementStyles.top
				? ((popoverHeight - Ruler.height( this.$el )) / 2) + parseFloat( elementStyles.top )
				: 0;
			this.arrowTop = triggerTop + Math.min( triggerHeight / 2, popoverHeight / 2 ) - extraSpacing + 'px';
		}
