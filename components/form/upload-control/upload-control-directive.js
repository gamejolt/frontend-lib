angular
	.module('gj.Form.UploadControl')
	.directive('gjFormUploadControl', function(
		$q,
		$document,
		$window,
		$parse,
				...
				function setupGeometryValidators() {
					...
					if (geometryToCheck.length) {
						...
						if (geometryKeys['width'] || geometryKeys['height']) {
							ngModel.$asyncValidators.dimensions = setupGeometryValidator(
								function(dimensions) {
									if (geometryVals['width'] && geometryVals['height']) {
										return (
											dimensions[0] == geometryVals['width'] &&
											dimensions[1] == geometryVals['height']
										);
							...
							if (geometryKeys['min-width'] || geometryKeys['min-height']) {
								ngModel.$asyncValidators.minDimensions = setupGeometryValidator(
									function(dimensions) {
										if (
											geometryVals['min-width'] &&
											geometryVals['min-height']
										) {
											return (
												dimensions[0] >= geometryVals['min-width'] &&
												dimensions[1] >= geometryVals['min-height']
											);
							...
							if (geometryKeys['max-width'] || geometryKeys['max-height']) {
								ngModel.$asyncValidators.maxDimensions = setupGeometryValidator(
									function(dimensions) {
										if (
											geometryVals['max-width'] &&
											geometryVals['max-height']
										) {
											return (
												dimensions[0] <= geometryVals['max-width'] &&
												dimensions[1] <= geometryVals['max-height']
											);
						...
						if (geometryKeys['ratio']) {
							...
							ngModel.$asyncValidators.minRatio = setupGeometryValidator(
								function(dimensions) {
									var ratio = dimensions[0] / dimensions[1];
