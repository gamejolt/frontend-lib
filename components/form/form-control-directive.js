angular.module( 'gj.Form' ).directive( 'formControl', function( $compile )
{
	...
	return {
		...
		compile: function( element, attrs )
		{
			...
			if ( attrs.formControl && [ 'radio', 'checkbox', 'file', 'upload', 'crop' ].indexOf( attrs.formControl ) === -1 ) {
			...
			if ( attrs.formControl && attrs.formControl != 'upload' && attrs.formControl != 'crop' ) {
			...
			return {
				post: function( scope, element, attrs, controllers )
				{
					...
					if ( angular.isUndefined( attrs.ngModel ) ) {
						element.attr( 'ng-model', gjForm.formModel + '[\'' + formGroup.name + '\']' );
					}
