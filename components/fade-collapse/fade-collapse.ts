...
export class AppFadeCollapse extends Vue
{
	...
	collapse()
	{
		...
		if ( this.isPrimed ) {
			...
			const scrollTo = Scroll.getElementOffsetFromContext( this.$el ) + this.collapseHeight - ExtraCollapsePadding;
	...
	private animStep()
	{
		...
		const curPos = Scroll.getElementOffsetFromContext( this.$el ) + this.$el.offsetHeight - ExtraCollapsePadding;
