angular
	.module('gj.Loading')
	.directive('gjLoading', function($interpolate, $parse) {
		return {
			restrict: 'E',
			scope: true,
			...
			link: function(scope, element, attrs) {
				scope.label = attrs.label
					? $interpolate(attrs.label)(scope)
					: 'Loading...';

				scope.hideLabel = attrs.hideLabel
					? $parse(attrs.hideLabel)(scope)
					: angular.isDefined(attrs.hideLabel);
				scope.isBig = attrs.loadingBig
					? $parse(attrs.loadingBig)(scope)
					: angular.isDefined(attrs.loadingBig);
				scope.noColor = attrs.loadingNoColor
					? $parse(attrs.loadingNoColor)(scope)
					: angular.isDefined(attrs.loadingNoColor);
				scope.isStationary = attrs.loadingStationary
					? $parse(attrs.loadingStationary)(scope)
					: angular.isDefined(attrs.loadingStationary);
