	...
	return {
		...
		controller: function(
			...
			if ($attrs.imgCropMinWidth || $attrs.imgCropMinHeight) {
				minSize = [
					$attrs.imgCropMinWidth
						? $parse($attrs.imgCropMinWidth)($scope.$parent)
						: 0,
					$attrs.imgCropMinHeight
						? $parse($attrs.imgCropMinHeight)($scope.$parent)
						: 0,
				];
			}
					...
					jqueryElement.Jcrop(
						{
							...
							aspectRatio: $attrs.imgCropAspectRatio
								? $parse($attrs.imgCropAspectRatio)($scope.$parent)
								: undefined,
						...
						function() {
							...
							var cropCoords = _this.ngModel.$isEmpty(_this.ngModel.$viewValue)
								? null
								: _this.ngModel.$viewValue;
							if (!cropCoords) {
