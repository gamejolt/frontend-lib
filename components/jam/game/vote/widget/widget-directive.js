angular.module( 'gj.Jam.Game.Vote.Widget' ).directive( 'gjJamGameVoteWidget', function()
{
	return {
		restrict: 'E',
		...
		controller: function( $scope, App, Environment, Api, Jam_VotingCategory )
		{
			...
			if ( App.user ) {
				...
				Api.sendRequest( '/jams-io/voting/' + this.jam.id + '/' + this.game.id, null, { detach: true } )
					.then( function( payload )
					{
						...
						if ( angular.isDefined( payload.votingCategories ) ) {
							_this.votingCategories = Jam_VotingCategory.populate( payload.votingCategories );
						}
