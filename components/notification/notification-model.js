	...
	$state, $q, $injector, $location, $window, Model, Api, Environment )
{
	function Notification( data )
	{
		...
		if ( this.from == 'User' ) {
			this.from_resource_model = new ($injector.get( 'User' ))( data.from_resource_model );
		}

		if ( this.to == 'Game' ) {
			this.to_resource_model = new ($injector.get( 'Game' ))( data.to_resource_model );
		}
		else if ( this.to == 'User' ) {
			this.to_resource_model = new ($injector.get( 'User' ))( data.to_resource_model );
		}
		else if ( this.to == 'Fireside_Post' ) {
			this.to_resource_model = new ($injector.get( 'Fireside_Post' ))( data.to_resource_model );
		}
		...
		if ( this.type == Notification.TYPE_COMMENT_ADD ) {
			this.action_resource_model = new ($injector.get( 'Comment' ))( data.action_resource_model );
			...
			this.action_resource_model = new ($injector.get( 'Forum_Post' ))( data.action_resource_model );
			...
			this.action_resource_model = new ($injector.get( 'User_Friendship' ))( data.action_resource_model );
			this.action_label = 'Friend Request';
			this.url = $state.href( 'profile.overview', { slug: this.from_resource_model.slug, id: this.from_resource_model.id } );
			...
			this.action_resource_model = new ($injector.get( 'Game_Rating' ))( data.action_resource_model );
			this.action_label = 'Game Rating';
			this.url = this.to_resource_model.getUrl();
			this.jolticon = 'jolticon-chart';
			this.is_game_based = true;
		}
		else if ( this.type == Notification.TYPE_GAME_FOLLOW ) {
			this.action_resource_model = new ($injector.get( 'GameLibrary_Game' ))( data.action_resource_model );
			this.action_label = 'Game Follow';
			this.url = this.from_resource_model.url;
			this.jolticon = 'jolticon-subscribe';
			this.is_user_based = true;
		}
		else if ( this.type == Notification.TYPE_GAME_NEWS_ADD ) {
			this.action_resource_model = new ($injector.get( 'Game_NewsArticle' ))( data.action_resource_model );
			this.action_label = 'News Update';
			this.url = $state.href( 'discover.games.view.news.view', {
				slug: this.to_resource_model.slug,
				id: this.to_resource_model.id,
				articleSlug: this.action_resource_model.slug,
				articleId: this.action_resource_model.id,
			} );
			this.jolticon = 'jolticon-blog-article';
			this.is_game_based = true;
		}
		else if ( this.type == Notification.TYPE_SELLABLE_SELL ) {
			this.action_resource_model = new ($injector.get( 'Order_Item' ))( data.action_resource_model );
	...
	Notification.prototype.go = function()
	{
		...
		// Need to fetch the URL first.
			...
			if ( this.type == Notification.TYPE_COMMENT_ADD || this.type == Notification.TYPE_COMMENT_ADD_OBJECT_OWNER ) {
				promise = $injector.get( 'Comment' ).getCommentUrl( this.action_resource_id );
			}
			else if ( this.type == Notification.TYPE_FORUM_POST_ADD ) {
				promise = $injector.get( 'Forum_Post' ).getPostUrl( this.action_resource_id );
			}
