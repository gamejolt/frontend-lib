...
{
	static Api: Api;
	static GameBuildFile: typeof GameBuildFile;
	static GameBuildParam: typeof GameBuildParam;
	...
	constructor( data?: any )
	{
		...
		if ( data.primary_file ) {
			this.primary_file = new GameBuild.GameBuildFile( data.primary_file );
		}

		this.params = [];
		if ( data.params && Array.isArray( data.params ) && data.params.length ) {
			this.params = GameBuild.GameBuildParam.populate( data.params );
		}
	...
	getDownloadUrl( options: { key?: string } = {} )
	{
		...
		return GameBuild.Api.sendRequest( '/web/discover/games/builds/get-download-url/' + this.id, data );
	}
	...
	{
		let params = [ this.game_id, this.game_package_id, this.game_release_id, this.id ];
...
const deps = { Api, GameBuildFile, GameBuildParam };
export const provideGameBuild = makeModel( GameBuild, deps );
