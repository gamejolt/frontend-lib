...
export default class AppAdVideo extends Vue
{
	...
	async mounted()
	{
		try {
			...
			switch ( e.getErrorCode() )
			{
				...
				case 403:
					Analytics.trackEvent( this.trackingLabel || 'Video Ad', 'Invalid Ad Format', e.getMessage() );
					break;

				default:
					Analytics.trackEvent( this.trackingLabel || 'Video Ad', 'Ad Error', e.getErrorCode() + ' - ' + e.getMessage() );
			}
		}
	...
	private bootstrap()
	{
		...
		return new Promise<any>( ( resolve, reject ) =>
		{
			...
			this.adContainerElem = this.$el.getElementsByClassName( 'ad-video-player-ad-container' )[0] as HTMLElement;
			...
			if ( Environment.env === 'development' ) {
				...
				adsRequest.adTagUrl = 'https://pubads.g.doubleclick.net/gampad/ads' +
					...
					'&impl=s&gdfp_req=1&env=vp&output=xml_vast2&unviewed_position_start=1' +
					'&url=[referrer_url]&correlator=[timestamp]' +
					'&cust_params=' + encodeURIComponent( 'ref_url=' + encodeURIComponent( window.location.href )  );
			}
